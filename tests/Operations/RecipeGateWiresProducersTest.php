 ...
 * (`GovernedDoor::open`) must carry the SAME `contractProducers` the agent gate's
 ...
 * `GovernedDoor::open()` itself does not touch the container — only `Kernel::root()` and
...
final class RecipeGateWiresProducersTest extends TestCase
{
    ...
    private function buildBridge(): object
    {
        ...
        // The door moved OUT of RecipeOperations: a recipe and a deployment share the motor and not
        // the meaning (greenhouse decisions/0223). What this test guards did not change — the gate a
        // sequence originates through must carry the same contract producers the agent gate does.
        /** @var object */
        return GovernedDoor::open($kernel, $root, $store, $session, 'apply recipe demo');
    }
