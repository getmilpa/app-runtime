...
final class SessionResultRecoveryTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->callSeq = $this->sessions->recordToolCall('own', 'source_read', ['path' => 'src/Domain.php'], '{"ok":true,"content":"complete stored bytes"}', true, false);
    ...
    public function testTheNativeProducerDeclaresTheRecordedResultContract(): void
    {
        $operations = (new SessionOperations(new DIContainer()))->operations();
        $reader = current(array_filter($operations, static fn ($operation) => $operation->name === 'agent:result'));
        self::assertInstanceOf(SessionResultOperation::class, $reader);
        self::assertFalse($reader->mutating);
        self::assertSame(['agent:read', 'agent:answer'], $reader->scopes);
    }

    public function testRecoveryOffersAndAdmitsAnOwnRecordedResultByContract(): void
    {
        $gate = $this->gate();
        self::assertSame(['source_read'], $gate->recoveryHiddenTools(['history_read', 'source_read']));
        self::assertNull($gate->refuse('history_read', ['session' => 'own', 'seq' => $this->callSeq]));
        self::assertNotNull($gate->refuse('source_read', ['path' => 'src/Domain.php']));
    }

    public function testANameAloneCannotDeclareARecordedResultReader(): void
    {
        $plain = new Operation(name: 'agent:result', description: 'A generic read with the same name', handler: static fn () => [], effects: EffectProfile::readOnly());
        $gate = $this->gate([$plain]);
        self::assertSame(['agent_result'], $gate->recoveryHiddenTools(['agent_result']));
        self::assertNotNull($gate->refuse('agent_result', ['session' => 'own', 'seq' => $this->callSeq]));
    }
    ...
    public function testReadingTheResultDoesNotClearRecovery(): void
    {
        $gate = $this->gate();
        self::assertNull($gate->refuse('history_read', ['session' => 'own', 'seq' => $this->callSeq]));
        $this->sessions->recordToolCall('own', 'history_read', ['session' => 'own', 'seq' => $this->callSeq], '{"ok":true,"content":"complete stored bytes"}', true, false);
    ...
    public function testRecordedResultAccessDoesNotBypassThePrerequisiteGate(): void
    {
        $gate = $this->gate(prerequisite: new PrerequisiteGate(['required_first']));
        $refusal = $gate->refuse('history_read', ['session' => 'own', 'seq' => $this->callSeq]);
    ...
    {
        $reader = new SessionResultOperation(name: 'history:read', description: 'An aliased declared result reader', handler: static fn () => [], effects: EffectProfile::readOnly());
