...
**Experimental branch — not ready for release.** Greenhouse evidence/0703 found that a terminal
tool refusal can reach the same return site as a final model answer and incorrectly acquire a
verified delivery closure. The prototype below is retained for that counterexample; typed
termination provenance is required before this contract can graduate.
...
At each natural end, the runtime reads `AcceptanceEvidence` again from the native stream,
...
a declaration retain the original recorded-work verdict. Paused, exhausted, stalled,
interrupted and truncated turns do not derive a closure.
