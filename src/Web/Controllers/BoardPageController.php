...
{
    /** Render the board for the session named in the query. */
    public function show(ServerRequestInterface $request): ResponseInterface
    {
        $params = $request->getQueryParams();
        $session = \is_string($params['session'] ?? null) ? $params['session'] : 'default';

        return new Response(200, ['Content-Type' => 'text/html; charset=utf-8'], (new BoardPage())->render($session, '/board/data'));
    }
}
