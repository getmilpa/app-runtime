...
/**
 * Bounded evidence of file consultations in one copied-app test. Known describes this scope,
 * never complete or semantic execution dependencies. Only the trusted observer constructs it.
 */
final readonly class TestInputWitness
{
    ...
    public static function unknown(TrialInputAttempt $attempt): self
    {
        return new self($attempt, 'unknown');
    }

    /** Validate correlation and the closed file-state vocabulary before the guard can use it.
     * @param array<string, mixed> $record
     */
    public static function fromObservation(TrialInputAttempt $attempt, array $record): self
    {
        if ($attempt->operation !== 'test'
        ...
        return new self($attempt, 'known', $inputs);
    }
